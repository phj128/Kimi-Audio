...
def extract_speech_token(model: WhisperVQEncoder, feature_extractor: WhisperFeatureExtractor, utts):
    ...
    with torch.no_grad():
        ...
        for start in range(0, len(audios), batch_size):
            ...
            features["input_features"] = features["input_features"].to(torch.cuda.current_device()).to(dtype) # (1, 128, 1000)
            features["attention_mask"] = features["attention_mask"].to(torch.cuda.current_device()) # (1, 1000)
            # import ipdb; ipdb.set_trace()
            outputs = model(**features)
            speech_tokens = outputs.quantized_token_ids # (1, 125)
            attention_mask = features.attention_mask[:, ::model.conv1.stride[0] * model.conv2.stride[0]]
            attention_mask = attention_mask[:, ::model.config.pooling_kernel_size] # (1, 125)
